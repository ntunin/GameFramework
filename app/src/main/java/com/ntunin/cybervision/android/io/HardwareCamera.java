...
import com.ntunin.cybervision.ercontext.PermissionListener;
 ...
 */
public class HardwareCamera implements PreviewCallback, Injectable {
    ...
    private boolean isPermissionGranted() {
        return ContextCompat.checkSelfPermission(ERContext.current(),
                Manifest.permission.CAMERA)
                == PackageManager.PERMISSION_GRANTED;
    }

    private boolean hasSystemFeature() {
        PackageManager pm = ERContext.current().getPackageManager();
        return pm.hasSystemFeature(PackageManager.FEATURE_CAMERA);
    }

    private void requestCameraPermissions() {
        ERContext.sendGrantRequest(
        ...
        synchronized (this) {
            ...
            try {
                ...
                if (sizes != null) {
                    ...
                    Size viewSize = (Size) ERContext.get(R.string.view_size);
    ...
    private ImageFrame createFrame() {
        ImageFrame frame = (ImageFrame) ERContext.create(Res.string(R.string.image_frame)).init(mFrameWidth, mFrameHeight);
        return frame;
    }
    ...
    private class CameraWorker implements Runnable {
        ...
        public void run() {
            do {
                ...
                synchronized (HardwareCamera.this) {
                    try {
                        while (!mCameraFrameReady && !mStopThread) {
                            HardwareCamera.this.wait();
                        }
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
    ...
    protected Size calculateCameraFrameSize(List<?> supportedSizes, JavaCameraSizeAccessor accessor, Size surfaceSize) {
        ...
        Size size = (Size) ERContext.create(R.string.int_size).init(calcWidth, calcHeight);
